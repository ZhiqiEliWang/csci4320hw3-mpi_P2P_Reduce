  ...
  if (taskid == MASTER) {
    ...
    for (i=1; i<=numworkers; i++)	{			
      ...
      MPI_Recv(&c[offset][0], count, MPI_DOUBLE, source, mtype, MPI_COMM_WORLD, 
               &status);
      printf("Master MPI Rank %d: worker %d completed work starting at offset %d\n",
...
//   printf ("offset =%d\n", offset);
    MPI_Recv(&rows, 1, MPI_INT, source, mtype, MPI_COMM_WORLD, &status);
   //printf ("row =%d\n", rows);
    printf ("Worker MPI Rank %d: Master =%d, mtype=%d, offset %d, rows %d\n",
	    taskid, source, mtype, offset, rows);
    count = rows*NCA;
...
//    printf ("b=\n");
    ...
    mtype = FROM_WORKER;
    printf ("MPI Rank %d: after compute\n", taskid);
    MPI_Send(&offset, 1, MPI_INT, MASTER, mtype, MPI_COMM_WORLD);
    MPI_Send(&rows, 1, MPI_INT, MASTER, mtype, MPI_COMM_WORLD);
    MPI_Send(&c, rows*NCB, MPI_DOUBLE, MASTER, mtype, MPI_COMM_WORLD);
    printf ("MPI Rank %d: after send\n", taskid);
